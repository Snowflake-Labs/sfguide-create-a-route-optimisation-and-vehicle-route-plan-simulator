...
ALTER TABLE IF EXISTS data.CA_TRAVEL_TIME_RES7 ADD COLUMN IF NOT EXISTS VEHICLE_TYPE VARCHAR DEFAULT 'cycling-electric';
...
ALTER TABLE IF EXISTS data.CA_TRAVEL_TIME_RES8 ADD COLUMN IF NOT EXISTS VEHICLE_TYPE VARCHAR DEFAULT 'cycling-electric';
...
ALTER TABLE IF EXISTS data.CA_TRAVEL_TIME_RES9 ADD COLUMN IF NOT EXISTS VEHICLE_TYPE VARCHAR DEFAULT 'cycling-electric';
...
CREATE OR REPLACE PROCEDURE data.BUILD_TRAVEL_TIME_RANGE_REGION(P_RES VARCHAR, P_START_SEQ INTEGER, P_END_SEQ INTEGER, P_MATRIX_FN VARCHAR, P_VEHICLE_PROFILE VARCHAR DEFAULT 'cycling-electric')
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
...
CREATE OR REPLACE PROCEDURE data.BUILD_MATRIX_FOR_REGION(P_RES VARCHAR, P_MIN_LAT FLOAT, P_MAX_LAT FLOAT, P_MIN_LON FLOAT, P_MAX_LON FLOAT, P_MATRIX_FN VARCHAR, P_REGION VARCHAR, P_VEHICLE_PROFILE VARCHAR DEFAULT 'cycling-electric')
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
...
CREATE OR REPLACE PROCEDURE data.FLATTEN_MATRIX_RAW(P_RES VARCHAR, P_REGION VARCHAR, P_VEHICLE_TYPE VARCHAR DEFAULT 'cycling-electric')
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
