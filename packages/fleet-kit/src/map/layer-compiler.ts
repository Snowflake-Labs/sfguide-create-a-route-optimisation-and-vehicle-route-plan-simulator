    ...
    if (spec.geojsonColumn && r[spec.geojsonColumn]) {
      try {
        ...
        if (Array.isArray(geo?.coordinates) && geo.coordinates.length > 1) {
          out.push({ ...r, path: decimateLineCoords(geo.coordinates as number[][], cap) });
          continue;
        }
 ...
 * about it.
 */
export function drawnCount(spec: LayerSpec, rows: Row[]): number {
...
export function encodingColumns(spec: LayerSpec): string[] {
  switch (spec.type) {
    case 'scatterplot':
    ...
    case 'path':
      return spec.geojsonColumn
        ? [spec.geojsonColumn]
        : [spec.start?.lng, spec.start?.lat, spec.end?.lng, spec.end?.lat].filter((c): c is string => !!c);
    case 'geojson':
      return [spec.geojsonColumn];
    default:
      return [];
...
export function compileLayerWithFit(
  spec: LayerSpec,
  rows: Row[],
  viewState: Record<string, unknown>,
  index: number,
  hovered?: { layerId: string; value: unknown } | null,
): { layer: Layer | null; fitCoords: [number, number][]; drawn: number } {
  if (!rows || rows.length === 0) return { layer: null, fitCoords: [], drawn: 0 };
...
function geoFeatures(spec: GeoJsonLayerSpec, rows: Row[]): GeoJSON.FeatureCollection {
  ...
  const push = (f: GeoJSON.Feature) => {
    if (f?.geometry) f = { ...f, geometry: decimateGeometry(f.geometry as any, cap) as any };
    features.push(f);
  };
  for (const r of rows) {
    ...
    try {
      ...
      if (geom?.type === 'FeatureCollection') for (const f of geom.features) push(f);
      else if (geom?.type === 'Feature') push(geom);
      else if (geom?.type) push({ type: 'Feature', geometry: decimateGeometry(geom, cap), properties: r });
...
function fitFromPathData(data: Array<{ path: number[][] }>): [number, number][] {
  const out: [number, number][] = [];
  for (const seg of data) for (const p of seg.path) out.push([p[0], p[1]]);
  return out;
}
