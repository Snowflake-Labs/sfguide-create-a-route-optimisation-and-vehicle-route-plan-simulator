...
import { detectGeoColumns } from './detect-geo';
 ...
 */
export function rebindLayerGeometry(layer, columns, rows) {
    const sample = rows ?? [];
    if (!sample.length || !columns?.length)
        return { layer };
    // Probe against the ROW KEY, since that is what the compiler indexes with.
    const probe = columns.map((c) => ({ name: c.key, type: c.type ?? '' }));
    switch (layer.type) {
        case 'geojson':
        case 'path': {
            ...
            if (layer.type === 'path' && !declared)
                return { layer };
            if (resolves(declared, sample))
                return { layer };
            const det = detectGeoColumns(probe, sample);
            if (det?.type !== 'geojson' || !det.geojsonColumn)
                return { layer };
            return {
        ...
        case 'h3': {
            if (resolves(layer.hexColumn, sample))
                return { layer };
            const det = detectGeoColumns(probe, sample);
            if (det?.type !== 'h3' || !det.hexColumn)
                return { layer };
            return {
        ...
        case 'scatterplot': {
            if (resolves(layer.lng, sample) && resolves(layer.lat, sample))
                return { layer };
            const det = detectGeoColumns(probe, sample);
            if (det?.type !== 'scatterplot' || !det.lng || !det.lat)
                return { layer };
            return {
...
//# sourceMappingURL=rebind-geometry.js.map