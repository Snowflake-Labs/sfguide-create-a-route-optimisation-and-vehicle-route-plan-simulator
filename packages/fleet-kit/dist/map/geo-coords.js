 ...
 */
export function clampFitPadding(p, width, height) {
    const ceiling = Math.floor(Math.min(width, height) * MAX_PADDING_FRACTION);
    const clamp = (v) => Math.max(0, Math.min(isFiniteNum(v) ? v : 0, ceiling));
...
function isInRange(lng, lat) {
    return (isFiniteNum(lng) && isFiniteNum(lat) && lng >= -180 && lng <= 180 && lat >= -90 && lat <= 90);
}
 ...
 */
export function coordsFromH3Cell(cell) {
 ...
 */
export function pushCoordPairs(c, out) {
    if (!Array.isArray(c))
        return;
    if (typeof c[0] === 'number' && typeof c[1] === 'number') {
        if (isInRange(c[0], c[1]))
            out.push([c[0], c[1]]);
        return;
    }
    for (const inner of c)
        pushCoordPairs(inner, out);
}
export function coordsFromH3Cells(rows, getCell, opts = {}) {
    ...
    for (let i = 0; i < rows.length; i += stride) {
        for (const c of coordsFromH3Cell(getCell(rows[i])))
            out.push(c);
    }
    return out;
}
...
//# sourceMappingURL=geo-coords.js.map