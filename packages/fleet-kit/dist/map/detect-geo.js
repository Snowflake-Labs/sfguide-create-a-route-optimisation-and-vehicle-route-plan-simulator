...
function isLatValue(v) {
    return typeof v === 'number' && Number.isFinite(v) && v >= -90 && v <= 90;
}
function isLonValue(v) {
    return typeof v === 'number' && Number.isFinite(v) && v >= -180 && v <= 180;
}
/** Non-null values of a column from the first `limit` rows. */
function sampleValues(rows, name, limit) {
    const out = [];
 ...
 */
function detectH3Column(columns, rows) {
    for (const col of columns) {
        if (!TEXT_TYPES.has(typeOf(col)))
            continue;
        const sample = sampleValues(rows, col.name, H3_SAMPLE_ROWS);
        if (sample.length === 0)
            continue;
        if (sample.every((v) => typeof v === 'string' && isValidCell(v)))
            return col.name;
    }
    return undefined;
}
...
function detectGeoJsonColumn(columns, rows) {
    ...
    for (const col of columns) {
        if (GEO_TYPES.has(typeOf(col)))
            return { name: col.name, declared: true };
    }
    // A GEOGRAPHY projected through ST_ASGEOJSON reports as TEXT/VARIANT, so it is
    // only recognisable by shape.
    for (const col of columns) {
        const t = typeOf(col);
        if (!TEXT_TYPES.has(t) && t !== 'variant' && t !== 'object')
            continue;
        const sample = sampleValues(rows, col.name, H3_SAMPLE_ROWS);
        if (sample.length === 0)
            continue;
        if (sample.every(looksLikeGeoJson))
            return { name: col.name, declared: false };
    }
    return undefined;
}
/**
 ...
 */
function detectLatLon(columns, rows) {
    const numeric = columns.filter((c) => NUMERIC_TYPES.has(typeOf(c)));
    let lat;
    let lon;
    for (const col of numeric) {
        const lower = col.name.toLowerCase();
        if (!lat && LAT_NAMES.has(lower))
            lat = col.name;
        if (!lon && LON_NAMES.has(lower))
            lon = col.name;
    }
    if (!lat || !lon || lat === lon)
        return undefined;
    ...
    if (latValues.length === 0 || lonValues.length === 0)
        return undefined;
    if (!latValues.every(isLatValue) || !lonValues.every(isLonValue))
        return undefined;
    return { lat, lon };
}
/**
 ...
 */
export function detectGeoColumns(columns, rows) {
    if (!columns?.length)
        return undefined;
    const sample = rows ?? [];
    ...
    if (hex)
        return { type: 'h3', hexColumn: hex, reason: `column ${hex} holds valid H3 cell ids` };
    if (declared) {
...
//# sourceMappingURL=detect-geo.js.map