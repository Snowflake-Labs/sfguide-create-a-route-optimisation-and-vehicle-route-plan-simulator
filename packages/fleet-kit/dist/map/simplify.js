 ...
 */
export function decimateLineCoords(coords, maxPoints = DEFAULT_MAX_PATH_POINTS) {
    if (!Array.isArray(coords) || maxPoints <= 0 || coords.length <= maxPoints)
        return coords;
 ...
 */
export function decimateGeometry(geom, maxPoints = DEFAULT_MAX_PATH_POINTS) {
    if (!geom || typeof geom !== 'object')
        return geom;
    switch (geom.type) {
        case 'LineString': {
            const next = decimateLineCoords(geom.coordinates, maxPoints);
