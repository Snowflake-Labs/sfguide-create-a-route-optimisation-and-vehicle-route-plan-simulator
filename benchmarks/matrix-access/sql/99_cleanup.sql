...
ALTER WAREHOUSE BENCH_INT_WH DROP TABLES (BENCHMARK.BENCH_MATRIX.BENCH_MATRIX_INTERACTIVE);

-- Drop the warehouses
DROP WAREHOUSE IF EXISTS BENCH_INT_WH;
DROP WAREHOUSE IF EXISTS BENCH_BUILD_WH;
DROP WAREHOUSE IF EXISTS BENCH_STD_WH;

-- Drop the schema and all benchmark tables
DROP SCHEMA IF EXISTS BENCHMARK.BENCH_MATRIX CASCADE;

-- Sanity check
SHOW SCHEMAS LIKE 'BENCH_MATRIX' IN DATABASE BENCHMARK;
