...
# --no-run, run a baseline evaluation against each. This is what clears the "Create the
# first eval set" / "Run an evaluation" items on the Snowsight agent setup checklist;
# run_agent_evals.py above is a DIFFERENT harness that tests the MCP verb path, which
# Snowsight evaluations cannot reach at all. Runs cost credits, so the installer calls
# this with --no-run unless RUN_AGENT_EVALS=1.
