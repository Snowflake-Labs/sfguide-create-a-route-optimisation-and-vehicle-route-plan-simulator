...
| `emergency-response` | demo | Single-page, multi-step evacuation-planning wizard. Step 1 colors a state's ZIP codes by FEMA NRI flood/wildfire risk; Step 2 seeds InnovAge PACE centers + Overture participant addresses inside the union of per-center drive-time isochrones; Step 3 sets per-center vehicles/capacity; Step 4 solves a capacitated multi-depot evacuation VRP (`OPTIMIZATION`) over participants at/above a chosen ZIP risk level. Fully client-driven via read-only `sfQuery`; risk = `V_ZIP_RISK` (NRI county risk joined to ZIP by county FIPS). States via `STATE_REGION_MAP` (CA/CO/PA). |
