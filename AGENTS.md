...
- **Cortex Agent evaluations never call MCP tools, so most of this stack cannot be scored by them**: per the evaluations docs, "the agent doesn't call any MCP server tool during the run". Every routing, ops and admin capability here is a synapse verb, so `answer_correctness` on those questions would mark the agent wrong for a platform limitation. The eval datasets in `fleet_sa_app/app/agent_evals.sql` therefore ask only what the Cortex Analyst + Cortex Search tools can answer, plus boundary/refusal and confirm-before-acting cases; `evals/ops_agent.yaml` and `evals/admin_agent.yaml` deliberately omit `answer_correctness` and score only `tool_selection_accuracy` + `logical_consistency`. The MCP verb path is covered by the separate live harness `.cortex/skills/evals/run_agent_evals.py`, whose `verb` assertions read `VERB_ATTEMPT`. The two are complementary - do not "fix" either by making it duplicate the other. Also note the run YAMLs carry no `dataset:` block (with one present, every re-run tries to recreate the dataset and fails) and pin explicit metric versions (scores from different metric versions are not comparable and Snowsight charts each version separately).
...
- **Synapse verb procs need `IDEMPOTENCY_KEY ... DEFAULT NULL` + agents must be recreated after any bundle redeploy**: The Cortex Agent MCP server calls each verb with NAMED args and omits the optional `idempotency_key`, so the trailing proc arg MUST have `DEFAULT NULL` (emitted by `fleet_tools/vendor/synapse/src/build/ddl.ts`); without it Snowflake raises "named arguments [...] do not match any signature" before the body runs and the agent reports a generic "Error parsing response" with NO `VERB_ATTEMPT` row. Separately, `npx synapse deploy` does `CREATE OR REPLACE MCP SERVER`, so any `install_synapse_bundles.sh` run (even out-of-band) MUST be followed by `create_agents.sh <connection>` - agents bind to the MCP server at creation time and go stale when it is replaced. This covers all FOUR agents, and `FLEET_SUPER_AGENT` goes stale when ANY of the three bundles is redeployed. Invariant: every agent `created_on` > its MCP server `created_on`. Full detail in `.cortex/skills/install-fleet-apps/references/synapse-bundles.md`.
- **`CREATE OR REPLACE AGENT` DROPS every grant on the agent, so recreating an agent revokes access**: this is the companion trap to the rule above, and it fires precisely when you follow that rule. `create_agents.sh` recreates all four agents, which silently drops their `USAGE` grants; the authoritative grant pass (`fleet_sa_app/app/role_binding.sql`) is install **step 8** while `create_agents.sh` is **step 6**. A full install therefore self-corrects and hides the defect, but a STANDALONE `create_agents.sh` run - exactly what a bundle redeploy requires - used to leave all four agents reachable by `ACCOUNTADMIN` only. The symptom is Snowsight showing "grant users access to the agent" unchecked, plus every app user losing the consumer `FLEET_AGENT`; nothing errors, because an agent with no grantee is a valid object. `create_agents.sh` now re-applies the four grants itself (each in its own exception handler, since on a fresh install the `FLEET_APP_*` roles do not exist yet at step 6) and prints a per-agent `OK`/`PENDING` line. `role_binding.sql` remains the single authoritative source - keep the two in sync, and never widen `FLEET_SUPER_AGENT` beyond `FLEET_APP_ADMIN` (that grant is the Tenet 3 boundary). Invariant: `SHOW GRANTS ON AGENT <name>` must return a `USAGE` row, not `OWNERSHIP` alone. Verifying a role cannot reach an agent needs `USE SECONDARY ROLES NONE`, or a secondary `ACCOUNTADMIN` silently authorizes the call.
