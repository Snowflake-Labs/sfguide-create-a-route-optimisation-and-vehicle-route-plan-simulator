...
-- 2e. FACT_DELIVERIES (Studio-generated, vehicle-agnostic deliveries)
CREATE TABLE IF NOT EXISTS SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES (
  OFFER_ID         VARCHAR,
  REGION           VARCHAR(100),
  VEHICLE_TYPE     VARCHAR(20),
  SOURCE           VARCHAR(30),
...
ALTER TABLE SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES ADD COLUMN IF NOT EXISTS VEHICLE_EQUIPMENT VARCHAR(30);
ALTER TABLE SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES ADD COLUMN IF NOT EXISTS DISTANCE_KM FLOAT;
ALTER TABLE SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES ADD COLUMN IF NOT EXISTS PRICE_PER_KM_USD FLOAT;
ALTER TABLE SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES ADD COLUMN IF NOT EXISTS PARTNER_ID VARCHAR;
ALTER TABLE SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES ADD COLUMN IF NOT EXISTS STATUS VARCHAR(20);

TRUNCATE TABLE IF EXISTS SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES;

COPY INTO SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES
FROM (
  SELECT
    $1:OFFER_ID::VARCHAR,
    $1:REGION::VARCHAR,
    ...
    $1:STATUS::VARCHAR
  FROM @OPENROUTESERVICE_APP.CORE.SEED_DATA_STAGE/synthetic_ebikes/fact_deliveries/
)
FILE_FORMAT = (TYPE = PARQUET)
PURGE = FALSE
FORCE = TRUE;
...
-- 2k. MARKETPLACE.FACT_DELIVERY_ROUTES (pre-computed delivery routes)
CREATE SCHEMA IF NOT EXISTS FLEET_INTELLIGENCE.MARKETPLACE
  COMMENT = '{"origin":"sf_sit-is-fleet","name":"oss-build-routing-solution","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}';

CREATE TABLE IF NOT EXISTS FLEET_INTELLIGENCE.MARKETPLACE.FACT_DELIVERY_ROUTES (
  ...
  JOB_ID       VARCHAR,
  CONSTRAINT PK_FACT_DELIVERY_ROUTES PRIMARY KEY (OFFER_ID)
)
COMMENT = '{"origin":"sf_sit-is-fleet","name":"oss-build-routing-solution","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}';

TRUNCATE TABLE IF EXISTS FLEET_INTELLIGENCE.MARKETPLACE.FACT_DELIVERY_ROUTES;

COPY INTO FLEET_INTELLIGENCE.MARKETPLACE.FACT_DELIVERY_ROUTES
FROM @OPENROUTESERVICE_APP.CORE.SEED_DATA_STAGE/synthetic_ebikes/fact_delivery_routes/
FILE_FORMAT = (TYPE = PARQUET)
MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE
PURGE = FALSE
FORCE = TRUE;
...
SET OFFER_TS_OFFSET = (
  SELECT TIMESTAMPDIFF('SECOND',
    (SELECT MAX(POSTED_AT) FROM SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES),
    CURRENT_TIMESTAMP())
);

UPDATE SYNTHETIC_DATASETS.UNIFIED.FACT_DELIVERIES
...
--   * FLEET_INTELLIGENCE.MARKETPLACE.CONFIG        -> Deliveries (VW_DELIVERY_ENRICHED)
