...
1. **Execute** notebook setup using SQL:
   ```sql
   ...
   CREATE OR REPLACE NOTEBOOK IDENTIFIER('OPENROUTESERVICE_SETUP.PUBLIC.DOWNLOAD_MAP') 
    FROM '@OPENROUTESERVICE_SETUP.PUBLIC.NOTEBOOOK'
    QUERY_WAREHOUSE = 'COMPUTE_WH' 
    RUNTIME_NAME = 'SYSTEM$BASIC_RUNTIME' 
    COMPUTE_POOL = 'OPENROUTESERVICE_NATIVE_APP' 
...
1. **Execute** notebook with country parameter:
   ...
   **Timeout:** Use 1200 seconds (map downloads can be large)
...
3. **Upload** modified file back to stage:
   ```sql
   PUT file:///tmp/ors-config.yml @OPENROUTESERVICE_NATIVE_APP.CORE.ORS_SPCS_STAGE OVERWRITE=TRUE
   ```
