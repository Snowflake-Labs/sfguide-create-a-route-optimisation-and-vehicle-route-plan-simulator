...
--   Step 2  Seed InnovAge centers + Overture participant addresses inside the
...
-- ----------------------------------------------------------------------------
-- Step 1: InnovAge centers (loaded from the committed geocoded CSV)
-- ----------------------------------------------------------------------------
-- The CSV ships with the skill at datasets/innovage_centers_geocoded.csv. The
...
CREATE TABLE IF NOT EXISTS EMERGENCY_RESPONSE.CORE.INNOVAGE_CENTERS (
...
TRUNCATE TABLE EMERGENCY_RESPONSE.CORE.INNOVAGE_CENTERS;
COPY INTO EMERGENCY_RESPONSE.CORE.INNOVAGE_CENTERS
...
FROM (
  SELECT
    ...
    $8::NUMBER, $9
  FROM @EMERGENCY_RESPONSE.CONFIG.SEED_STAGE/innovage_centers_geocoded.csv
)
...
DROP PROCEDURE IF EXISTS EMERGENCY_RESPONSE.CORE.GENERATE_INNOVAGE_DATASET(VARCHAR);
