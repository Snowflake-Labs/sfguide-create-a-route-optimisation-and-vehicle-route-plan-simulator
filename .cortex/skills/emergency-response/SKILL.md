...
5. **OpenCage secret** `OPENROUTESERVICE_APP.CORE.OPENCAGE_API_KEY` + `ORS_GEOCODE_EAI` external-access integration (egress `api.opencagedata.com:443`). Used **only at build time** to geocode the 18 InnovAge centers lacking coordinates; the resolved coordinates are committed to `datasets/innovage_centers_geocoded.csv`, so a fresh install needs no runtime geocoding.
...
| CREATE TABLE | DB `EMERGENCY_RESPONSE` | `INNOVAGE_CENTERS`, `STATE_REGION_MAP` |
...
EMERGENCY_RESPONSE.CORE.INNOVAGE_CENTERS        (20 centers, GEOGRAPHY LOC)
...
```bash
snow stage copy \
  .cortex/skills/emergency-response/datasets/innovage_centers_geocoded.csv \
  @EMERGENCY_RESPONSE.CONFIG.SEED_STAGE/ --overwrite -c <connection>
```

The pipeline installs FEMA NRI, creates `EMERGENCY_RESPONSE` (CONFIG/CORE/PIPELINE), seeds `STATE_REGION_MAP`, loads `INNOVAGE_CENTERS` (20 rows), builds `V_ZIP_RISK` and `ORS_ISOCHRONE_FOR_CENTER`, and drops retired v1 objects. Every CREATE carries the COMMENT tracking tag.

### Step 2 — Verify

```sql
SELECT COUNT(*) FROM EMERGENCY_RESPONSE.CORE.INNOVAGE_CENTERS;                              -- 20
...
The 18 centers without coordinates were geocoded once with a transient Python UDF that reads the `OPENCAGE_API_KEY` secret through `ORS_GEOCODE_EAI`, and the results were committed to `datasets/innovage_centers_geocoded.csv`. To re-geocode (e.g. new centers), recreate that UDF, run it over the new addresses, write the coordinates back into the CSV, and drop the UDF. No runtime geocoding occurs.
