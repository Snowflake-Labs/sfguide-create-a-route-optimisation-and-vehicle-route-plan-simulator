# Data Generation Reference (Steps 4-9)

Substitute `{PLACEHOLDER}` values based on user's chosen configuration. Execute each SQL as a separate `snowflake_sql_execute` call.

> **Filter Substitution:**
> - **US cities:** `{COUNTRY}` = `US`, `{STATE_FILTER}` = `ADDRESSES[0]:region::STRING = '{STATE}'`, `{ADDRESS_STATE_FILTER}` = `ADDRESS_LEVELS[0]:value::STRING = '{STATE}'`
> - **International:** `{COUNTRY}` = country code, `{STATE_FILTER}` = `1=1`, `{ADDRESS_STATE_FILTER}` = `1=1`
...
**WARNING:** ~500 orders: 2-4 min, ~1500: 8-12 min, ~3000: 20-30 min.

> **CRITICAL:** Use city-specific `DIRECTIONS_{LOCATION}()`, NOT generic `DIRECTIONS()`. The generic function routes to the default ORS service (Karlsruhe/Germany graph) and will fail for non-Karlsruhe coordinates.
> Replace `{LOCATION}` with the ORS region name in PascalCase (e.g., `SanFrancisco`, `London`, `NewYork`).
...
SELECT 
    ...
    FLEET_INTELLIGENCE_APP.ROUTING.DIRECTIONS_{LOCATION}(
