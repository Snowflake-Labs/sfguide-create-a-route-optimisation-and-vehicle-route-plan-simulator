...
with st.sidebar:
    ...
    if len(couriers_df) > 0:
        courier = st.selectbox('Choose Courier:', couriers_df)
    else:
        st.warning(f"No couriers found in {selected_city}")
        st.stop()
...
<h1grey>{CITY["name"]} — Viewing Deliveries for Courier {courier}</h1grey>
