...
The pain point behind this page is *information overload*: dispatchers juggle
several external load-board tabs plus their internal scheduling tool, manually
copying offers across, with no single canonical source of truth for either
*price* or *counterparty trust*.

`backload-matching` answers the *fleet-wide* version of that problem: one
OPTIMIZATION call replaces many dispatchers portal-hopping independently. But the
dispatcher still has the *individual* portal-hopping problem when they need to
pick a specific load, vet a specific carrier, or compare a price to the market.
...
## How operational signals map to the schema

| Signal | Schema response |
|---|---|
| *"Information overload across several portals"* | `VW_OFFER_ENRICHED` is one denormalized read; one filter UI replaces many. |
...
| *"Minutes, not overnight"* | `RATE_INDEX` is a Dynamic Table at 15-min target lag (configurable). For Phase C, `VW_OFFERS` flips to a DT over a Snowpipe Streaming feed. |
...
- `DIM_PARTNERS.COUNTRY` is region-aware (DE/NL/PL/CZ/AT for a Germany preset,
  FR/BE/ES/IT for France, US/CA/MX for North America, etc.).
...
| `AI_PARSE_DOCUMENT` (Phase D) | Transport-document, proof-of-delivery and hazmat-certificate extraction. |
