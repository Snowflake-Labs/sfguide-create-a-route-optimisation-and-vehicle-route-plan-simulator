...
-- POI joins carry REGION: LOCATION_ID repeats across regions (483 measured), so
-- an id-only join fans out one trailer into several rows and can pull another
-- region's copy of the place.
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.DIM_POIS h ON h.LOCATION_ID = f.HOME_LOCATION_ID AND h.REGION = f.REGION
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.DIM_POIS d ON d.LOCATION_ID = ld.DROPOFF_POI_ID AND d.REGION = ld.REGION
...
SELECT
  ...
  FALSE                                                                       AS HAZMAT
FROM SYNTHETIC_DATASETS.UNIFIED.FACT_TRIPS t
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.DIM_POIS o ON o.LOCATION_ID = t.ORIGIN_POI_ID AND o.REGION = t.REGION
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.DIM_POIS d ON d.LOCATION_ID = t.DESTINATION_POI_ID AND d.REGION = t.REGION
...
SELECT
  ...
  f.LISTING_TEXT
FROM shifted f
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.DIM_POIS p ON p.LOCATION_ID = f.PICKUP_POI_ID AND p.REGION = f.REGION
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.DIM_POIS d ON d.LOCATION_ID = f.DROPOFF_POI_ID AND d.REGION = f.REGION
