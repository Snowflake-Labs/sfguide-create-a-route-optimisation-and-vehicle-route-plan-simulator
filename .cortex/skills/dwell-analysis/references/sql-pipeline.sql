...
-- Auto-suspend dynamic tables after 2 hours to prevent credit consumption
-- with static seed data. Attendees can re-enable with ALTER DYNAMIC TABLE ... RESUME.
CREATE OR REPLACE TASK FLEET_INTELLIGENCE.DWELL_ANALYSIS.AUTO_SUSPEND_DTS
  WAREHOUSE = ROUTING_ANALYTICS
  SCHEDULE = '120 MINUTE'
  COMMENT = 'Auto-suspends dwell analysis DTs after 2 hours to save credits with static data'
AS
BEGIN
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_STATE_CHANGES SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_DWELL_SESSIONS SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_H3_CONGESTION SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_FACILITY_UTILIZATION SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_SLA_BREACHES SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_FLEET_DAILY_TRENDS SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_ACTIVE_DWELLS SUSPEND;
  ALTER DYNAMIC TABLE FLEET_INTELLIGENCE.DWELL_ANALYSIS.DT_SESSION_ENRICHED SUSPEND;
  ALTER TASK FLEET_INTELLIGENCE.DWELL_ANALYSIS.AUTO_SUSPEND_DTS SUSPEND;
END;

ALTER TASK FLEET_INTELLIGENCE.DWELL_ANALYSIS.AUTO_SUSPEND_DTS RESUME;
