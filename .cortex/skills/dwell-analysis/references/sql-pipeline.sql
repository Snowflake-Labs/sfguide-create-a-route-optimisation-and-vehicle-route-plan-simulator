...
-- Per-vehicle SLA: thresholds come from the catalog keyed by the active CONFIG
-- vehicle type (not the location-only SLA_THRESHOLDS table), so a preset switch
-- (hgv -> ebike) retailors warn/critical minutes. SLA_THRESHOLDS remains as a
-- seeded fallback for any other reader. WARNING_MIN/CRITICAL_MIN are aliased to
-- the historical WARNING_MINUTES/CRITICAL_MINUTES output names (downstream unchanged).
JOIN (
  SELECT LOCATION_TYPE, WARNING_MIN AS WARNING_MINUTES, CRITICAL_MIN AS CRITICAL_MINUTES
  FROM FLEET_INTELLIGENCE.CORE.DIM_VEHICLE_DWELL_SLA
...
WHERE e.DWELL_MINUTES >= t.WARNING_MINUTES AND (e.STATUS LIKE 'DWELL%' OR e.STATUS = 'IDLE');
