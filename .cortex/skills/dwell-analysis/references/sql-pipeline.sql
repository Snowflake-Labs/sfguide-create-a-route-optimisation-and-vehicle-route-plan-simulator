...
CREATE OR REPLACE VIEW FLEET_INTELLIGENCE.DWELL_ANALYSIS.VW_TRUCK_TELEMETRY AS
SELECT
    t.TELEMETRY_ID,
...
CREATE OR REPLACE VIEW FLEET_INTELLIGENCE.DWELL_ANALYSIS.VW_TRUCK_FLEET AS
SELECT
...
FROM FLEET_INTELLIGENCE.DWELL_ANALYSIS.VW_TRUCK_TELEMETRY
...
SELECT
  TRUCK_ID, SESSION_ID, TRIP_ID, STATUS, LOCATION_ID,
...
GROUP BY TRUCK_ID, SESSION_ID, TRIP_ID, STATUS, LOCATION_ID;
...
AS
SELECT
  s.TRUCK_ID, s.SESSION_ID, s.TRIP_ID, s.STATUS, s.LOCATION_ID,
...
LEFT JOIN FLEET_INTELLIGENCE.DWELL_ANALYSIS.VW_TRUCK_FLEET tf ON s.TRUCK_ID = tf.TRUCK_ID;
...
-- NOTE: Stream on VW_TRUCK_TELEMETRY is not supported because the view uses subquery expressions
...
--   ON VIEW FLEET_INTELLIGENCE.DWELL_ANALYSIS.VW_TRUCK_TELEMETRY
--   APPEND_ONLY = TRUE;
