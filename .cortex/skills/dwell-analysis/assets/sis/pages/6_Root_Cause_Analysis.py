...
with col_left:
    ...
    if len(profile_dwell) > 0:
        grouped = alt.Chart(profile_dwell).mark_bar().encode(
            x=alt.X('STATUS:N', title='Dwell type'),
            y=alt.Y('AVG_DWELL:Q', title='Avg dwell (min)'),
            color=alt.Color('DRIVER_PROFILE:N', scale=alt.Scale(scheme='category10'), title='Profile'),
            column=alt.Column('DRIVER_PROFILE:N', title='Driver profile',
                              sort=['COMPLIANT', 'MILD', 'OUTLIER']),
...
if len(dow_data) > 0:
    ...
    heatmap_metric = st.radio("Heatmap metric", ["SESSION_COUNT", "AVG_DWELL"])
