...
```sql
SHOW STREAMLITS IN SCHEMA OPENROUTESERVICE_NATIVE_APP.RETAIL_CATCHMENT_DEMO;
```
...
```sql
SELECT CONCAT('https://app.snowflake.com/', CURRENT_ORGANIZATION_NAME(), '/', CURRENT_ACCOUNT_NAME(), '/#/streamlit-apps/OPENROUTESERVICE_NATIVE_APP.RETAIL_CATCHMENT_DEMO.RETAIL_CATCHMENT_APP') AS streamlit_url;
```
