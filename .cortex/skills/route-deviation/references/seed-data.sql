...
SELECT
    ...
    COALESCE(t.PLANNED_ROUTE_GEOG, t.ROUTE_GEOG) AS EXPECTED_PATH,
    t.DISTANCE_KM AS ACTUAL_DISTANCE_KM,
    COALESCE(t.PLANNED_DISTANCE_KM, t.DISTANCE_KM * (1 - (HASH(t.TRIP_ID) % 20) / 100.0)) AS EXPECTED_DISTANCE_KM,
    CASE WHEN t.PLANNED_DISTANCE_KM > 0 AND t.PLANNED_DISTANCE_KM IS NOT NULL
         THEN ROUND(ABS(t.DISTANCE_KM - t.PLANNED_DISTANCE_KM) * 100.0 / t.PLANNED_DISTANCE_KM, 1)
         ELSE ROUND(ABS(HASH(t.TRIP_ID) % 1500) / 100.0, 1)
    END AS DISTANCE_DEVIATION_PCT,
    CASE WHEN t.DURATION_MINUTES > 0 AND t.PLANNED_DISTANCE_KM > 0
