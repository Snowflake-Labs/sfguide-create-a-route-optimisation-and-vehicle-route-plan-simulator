...
## Data Studio Projection Views

These views allow the skill to read from the unified Data Studio tables (`SYNTHETIC_DATASETS.UNIFIED`), filtered to HGV data only.

### VW_TRUCK_TELEMETRY

```sql
CREATE OR REPLACE VIEW FLEET_INTELLIGENCE.ROUTE_DEVIATION.VW_TRUCK_TELEMETRY AS
SELECT
    TELEMETRY_ID,
...
```sql
CREATE OR REPLACE VIEW FLEET_INTELLIGENCE.ROUTE_DEVIATION.VW_TRIP_DEVIATION AS
SELECT
    TRIP_ID,
    VEHICLE_ID AS TRUCK_ID,
    DRIVER_ID,
    ...
    DESTINATION_LAT,
    DESTINATION_LON,
    ROUTE_GEOG AS ACTUAL_PATH,
    DISTANCE_KM AS ACTUAL_DISTANCE_KM,
    DURATION_MINUTES AS ACTUAL_DURATION_MIN,
    PLANNED_ROUTE_GEOG AS EXPECTED_PATH,
    PLANNED_DISTANCE_KM AS EXPECTED_DISTANCE_KM,
    IS_DETOUR,
    DETOUR_DISTANCE_KM,
    TRIP_START,
    TRIP_END,
