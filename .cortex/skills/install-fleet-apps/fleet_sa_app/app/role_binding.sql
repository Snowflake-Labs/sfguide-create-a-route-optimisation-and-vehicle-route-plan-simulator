...
-- PLAN_STANDARDS: route-plan standards + planner variance. Needed here for the
-- same reason as the four above - F_ROUTE_RESOLVE_GAP is an owner's-rights
-- wrapper over the ORS optimizer, so an agent asking "what would the optimizer
-- have done with this route" gets "Unknown user-defined table function" without
-- this USAGE, which reads as a hallucinated object rather than a missing grant.
GRANT USAGE ON SCHEMA FLEET_APP.PLAN_STANDARDS      TO ROLE FLEET_APP_DYNAMIC_READER;
...
GRANT USAGE  ON ALL FUNCTIONS    IN SCHEMA FLEET_APP.PLAN_STANDARDS     TO ROLE FLEET_APP_DYNAMIC_READER;
GRANT USAGE  ON FUTURE FUNCTIONS IN SCHEMA FLEET_APP.PLAN_STANDARDS     TO ROLE FLEET_APP_DYNAMIC_READER;
...
GRANT SELECT ON ALL VIEWS    IN SCHEMA FLEET_APP.PLAN_STANDARDS     TO ROLE FLEET_APP_DYNAMIC_READER;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA FLEET_APP.PLAN_STANDARDS     TO ROLE FLEET_APP_DYNAMIC_READER;
-- The STANDARDS table itself is readable too: an agent asked "what IS the
-- standard" must be able to quote the thresholds rather than infer them from
-- which routes happened to breach.
GRANT SELECT ON ALL TABLES    IN SCHEMA FLEET_APP.PLAN_STANDARDS    TO ROLE FLEET_APP_DYNAMIC_READER;
GRANT SELECT ON FUTURE TABLES IN SCHEMA FLEET_APP.PLAN_STANDARDS    TO ROLE FLEET_APP_DYNAMIC_READER;
