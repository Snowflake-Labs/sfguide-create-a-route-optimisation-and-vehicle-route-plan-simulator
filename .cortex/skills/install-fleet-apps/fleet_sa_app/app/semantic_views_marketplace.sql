-- semantic_views_marketplace.sql - SV_OFFERS, the deliveries-marketplace SV.
-- Owned by install-fleet-apps. Kept in its own file, NOT appended to
-- semantic_views.sql, on purpose.
--
-- WHY A SEPARATE FILE
-- -------------------
-- SV_OFFERS reads FLEET_INTELLIGENCE.MARKETPLACE.VW_OFFER_ENRICHED and
-- VW_LANE_HISTORY, and NEITHER is built by this installer: those views are
-- created by the admin app's boot init (server/lib/init.ts) and by the
-- freight-exchange demo skill. On a fresh install the schema does not exist yet.
--
-- `snow sql -f` stops at the first failing statement and silently skips every
-- statement below it, so putting a hard CREATE for this view inside
-- semantic_views.sql would take the whole semantic layer down with it on any
-- deployment that has not booted the admin app or run freight-exchange. A
-- guarded in-SQL branch was the alternative, but the DDL below is full of single
-- quotes (COMMENT, WITH SYNONYMS), so wrapping it in EXECUTE IMMEDIATE would
-- require escaping every one of them - exactly the fragile construct AGENTS.md
-- warns about.
--
-- One file, one statement, run best-effort by installer step 4.5 means a missing
-- marketplace layer costs this one view and nothing else.
--
-- Idempotent (CREATE OR REPLACE). Re-run after the marketplace views appear.

ALTER SESSION SET query_tag = '{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql","module":"semantic_views_marketplace"}}';
...
-- No GRANTs here on purpose. role_binding.sql (installer step 6, which runs after
-- this file) already issues GRANT SELECT ON ALL + FUTURE SEMANTIC VIEWS IN SCHEMA
-- FLEET_INTELLIGENCE.SEMANTIC, so this view is covered whether it is created
-- before or after that step. Granting here as well would add nothing and would
-- abort this file if a role does not exist yet, losing the view we just created.
