...
--      globally).
--   3. last_drop is keyed on (REGION, VEHICLE_ID). VEHICLE_ID is unique only
--      WITHIN a region - measured on tib85385, V-DRI-00013 and V-DRI-00019
--      exist in both UsTexas and UnitedStatesOfAmerica - so partitioning on
--      VEHICLE_ID alone handed a Texas trailer the globally-latest trip,
--      which was a USA trip ending in Florida, ~900 km outside the Texas
--      road graph. ORS then refused the whole solve with code 6010. Same
--      reason the POI joins below carry REGION: 483 LOCATION_IDs appear in
--      more than one region, so an id-only join fans out one trailer into
--      several rows and can pull another region's copy of the place.
WITH last_drop AS (
...
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.V_DIM_POIS_CURRENT h ON h.LOCATION_ID = f.HOME_LOCATION_ID AND h.REGION = f.REGION
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.V_DIM_POIS_CURRENT d ON d.LOCATION_ID = ld.DROPOFF_POI_ID AND d.REGION = ld.REGION;
...
-- `poi` CTE instead. The POI joins below are ALSO region-keyed: LOCATION_ID
-- repeats across regions (483 measured), and without REGION the name could
-- come from another region's copy of the same place.
WITH p AS (
  SELECT
...
deduped AS (
  ...
  LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.V_DIM_POIS_CURRENT o ON o.LOCATION_ID = t.ORIGIN_POI_ID AND o.REGION = t.REGION
  LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.V_DIM_POIS_CURRENT d ON d.LOCATION_ID = t.DESTINATION_POI_ID AND d.REGION = t.REGION
...
LEFT JOIN SYNTHETIC_DATASETS.UNIFIED.V_DIM_POIS_CURRENT p2 ON p2.LOCATION_ID = f.PICKUP_POI_ID AND p2.REGION = f.REGION
