...
CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_VEHICLE_COST_OF_IDLENESS
  COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}' AS
SELECT
  t.VEHICLE_ID, t.REGION, t.REGION_LABEL, t.LAST_LOCATION_NAME, t.LAST_LOCATION_TYPE,
...
CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_LANE_DEMAND
  COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}' AS
-- Recency window is computed PER REGION. A single global MAX(TRIP_START)
-- would let the region with the freshest trips silently prune every other
-- region out of the 30-day window - the exact class of cross-region
-- contamination that de-scoping this layer could otherwise introduce.
WITH window_bounds AS (
  SELECT REGION, MAX(TRIP_START) AS MAX_TS FROM FLEET_APP.ROUTE_OPTIMIZATION.VW_TRIPS_CURRENT GROUP BY REGION
),
recent_trips AS (
  SELECT t.* FROM FLEET_APP.ROUTE_OPTIMIZATION.VW_TRIPS_CURRENT t
...
SELECT
  p.LOCATION_ID AS TERMINAL_ID, p.REGION,
