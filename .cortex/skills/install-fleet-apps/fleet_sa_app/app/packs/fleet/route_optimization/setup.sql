...
CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_DWELL_ENRICHED AS
SELECT * FROM FLEET_APP.DWELL.VW_DWELL_SESSIONS;

CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_FLEET_CURRENT AS
...
CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_TRIPS_CURRENT AS
SELECT TRIP_START, ORIGIN_POI_ID, DESTINATION_POI_ID
FROM SYNTHETIC_DATASETS.UNIFIED.V_FACT_TRIPS_CURRENT
WHERE VEHICLE_TYPE = (SELECT VEHICLE_TYPE FROM FLEET_INTELLIGENCE.ROUTE_OPTIMIZATION.CONFIG LIMIT 1)
  AND REGION = (SELECT REGION FROM FLEET_INTELLIGENCE.ROUTE_OPTIMIZATION.CONFIG LIMIT 1);

CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_POIS_CURRENT AS
SELECT LOCATION_ID, REGION, NAME, LOCATION_TYPE, POINT_GEOM, LAT, LNG
FROM SYNTHETIC_DATASETS.UNIFIED.V_DIM_POIS_CURRENT;

CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_CONFIG AS
SELECT * FROM FLEET_INTELLIGENCE.ROUTE_OPTIMIZATION.CONFIG;

CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_IDLE_VEHICLES AS
...
CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_FLEET_VEHICLE_PROFILE AS
WITH filtered AS (
...
CREATE OR REPLACE VIEW FLEET_APP.ROUTE_OPTIMIZATION.VW_VEHICLE_COST_OF_IDLENESS AS
SELECT
...
FROM FLEET_APP.ROUTE_OPTIMIZATION.VW_IDLE_VEHICLES t
