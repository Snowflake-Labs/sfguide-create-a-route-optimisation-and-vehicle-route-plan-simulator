...
RETURNS TABLE (
  ...
  PAID_HOURS NUMBER(12,4), DRIVE_HOURS NUMBER(12,4), IDLE_HOURS NUMBER(12,4), DRIVE_SHARE FLOAT,
  TRIPS NUMBER, DISTANCE_KM NUMBER(12,2),
  REGION VARCHAR, REGION_LABEL VARCHAR, VEHICLE_TYPE VARCHAR
)
COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-labor-overtime","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS
$$
  ...
  SELECT
    ...
    ROUND(DATEDIFF('second', MIN(g.TRIP_START), MAX(g.TRIP_END)) / 3600.0, 4)::NUMBER(12,4) AS PAID_HOURS,
    ...
    LEAST(ROUND(DATEDIFF('second', MIN(g.TRIP_START), MAX(g.TRIP_END)) / 3600.0, 4),
          ROUND(SUM(g.EFF_DRIVE_MIN) / 60.0, 4))::NUMBER(12,4) AS DRIVE_HOURS,
    -- On the clock but not driving.
    GREATEST(0, ROUND(DATEDIFF('second', MIN(g.TRIP_START), MAX(g.TRIP_END)) / 3600.0
                      - SUM(g.EFF_DRIVE_MIN) / 60.0, 4))::NUMBER(12,4) AS IDLE_HOURS,
    ...
    COUNT(*)                                               AS TRIPS,
    ROUND(SUM(g.DISTANCE_KM), 2)::NUMBER(12,2)             AS DISTANCE_KM,
    g.REGION                                               AS REGION,
...
RETURNS TABLE (
  ...
  HOURS_TO_DATE NUMBER(12,2), DAYS_WORKED NUMBER, DAYS_ELAPSED NUMBER, DAYS_REMAINING NUMBER,
  IS_CURRENT_WEEK BOOLEAN, IS_PARTIAL_START BOOLEAN, IS_PARTIAL_END BOOLEAN, PROJECTED_WEEK_HOURS NUMBER(12,2),
  CONTRACTED_HOURS_PER_WEEK FLOAT, STRAIGHT_HOURS NUMBER(12,2), OT_HOURS NUMBER(12,2),
  PROJECTED_OT_HOURS NUMBER(12,2), HOURLY_RATE FLOAT, EST_OT_COST NUMBER(12,2),
  ...
  DRIVE_HOURS NUMBER(12,2), DRIVE_SHARE_OF_PAID FLOAT, TRIPS NUMBER, DISTANCE_KM NUMBER(12,2),
...
$$
  ...
  SELECT
    ...
    o.SHIFT_TYPE,
    o.DRIVER_PROFILE,
    -- EXACT DECIMAL, NOT FLOAT, for every column an agent or a tile will SUM.
    -- ROUND(x, 2)::FLOAT looks rounded and is not: 2dp is not representable in
    -- binary, so the error accumulates over the rows of a GROUP BY and the total
    -- serializes as 2042.769999999999. MEASURED on this data: 37 of 120 grouped
    -- sums across five of these columns carried an artifact as FLOAT, and 0 as
    -- NUMBER. The RETURNS TABLE declaration above must carry the same type - it
    -- COERCES, so casting here alone would be a silent no-op.
    --
    -- Applied only to the additive columns. The 4dp ratios below stay FLOAT: they
    -- are averaged rather than summed, and their display precision is handled by
    -- lib/format-number.ts.
    ROUND(p.HOURS_TO_DATE, 2)::NUMBER(12,2)                   AS HOURS_TO_DATE,
    p.DAYS_WORKED,
    ...
    ROUND(p.PROJ_HOURS, 2)::NUMBER(12,2)                      AS PROJECTED_WEEK_HOURS,
    o.CONTRACTED_HOURS_PER_WEEK,
    ROUND(LEAST(p.HOURS_TO_DATE, p.T1), 2)::NUMBER(12,2)      AS STRAIGHT_HOURS,
    ROUND(GREATEST(0, p.HOURS_TO_DATE - p.T1), 2)::NUMBER(12,2) AS OT_HOURS,
    ROUND(GREATEST(0, p.PROJ_HOURS - p.T1), 2)::NUMBER(12,2)  AS PROJECTED_OT_HOURS,
    o.HOURLY_RATE,
    ROUND(GREATEST(0, p.PROJ_HOURS - p.T1) * o.HOURLY_RATE * o.OT_MULTIPLIER, 2)::NUMBER(12,2) AS EST_OT_COST,
    ...
    ROUND(GREATEST(0, p.PROJ_HOURS - p.T1) * o.HOURLY_RATE * GREATEST(0, o.OT_MULTIPLIER - 1), 2)::NUMBER(12,2) AS EST_OT_PREMIUM,
    o.CURRENCY_CODE,
