...
CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.PROVIDERS.ORS_SNAP_RAW(METHOD VARCHAR, OPTIONS VARIANT, REGION VARCHAR)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$ OPENROUTESERVICE_APP.CORE._SNAP_RAW(METHOD, OPTIONS, REGION) $$;

CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.PROVIDERS.ORS_MATCH_RAW(METHOD VARCHAR, OPTIONS VARIANT, REGION VARCHAR)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$ OPENROUTESERVICE_APP.CORE._MATCH_RAW(METHOD, OPTIONS, REGION) $$;

CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.PROVIDERS.ORS_EXPORT_RAW(METHOD VARCHAR, OPTIONS VARIANT, REGION VARCHAR)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$ OPENROUTESERVICE_APP.CORE._EXPORT_RAW(METHOD, OPTIONS, REGION) $$;
...
CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.CONTRACT._DISPATCH_SNAP(METHOD VARCHAR, OPTIONS VARIANT, REGION VARCHAR, PROVIDER VARCHAR)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$
  CASE ROUTING_PLATFORM.CONTRACT.RESOLVE_PROVIDER(REGION, PROVIDER)
    WHEN 'ors_internal' THEN ROUTING_PLATFORM.PROVIDERS.ORS_SNAP_RAW(METHOD, OPTIONS, REGION)
    ELSE ROUTING_PLATFORM.PROVIDERS.EXT_UNSUPPORTED('snap', ROUTING_PLATFORM.CONTRACT.RESOLVE_PROVIDER(REGION, PROVIDER))
  END
$$;

CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.CONTRACT._DISPATCH_MATCH(METHOD VARCHAR, OPTIONS VARIANT, REGION VARCHAR, PROVIDER VARCHAR)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$
  CASE ROUTING_PLATFORM.CONTRACT.RESOLVE_PROVIDER(REGION, PROVIDER)
    WHEN 'ors_internal' THEN ROUTING_PLATFORM.PROVIDERS.ORS_MATCH_RAW(METHOD, OPTIONS, REGION)
    ELSE ROUTING_PLATFORM.PROVIDERS.EXT_UNSUPPORTED('match', ROUTING_PLATFORM.CONTRACT.RESOLVE_PROVIDER(REGION, PROVIDER))
  END
$$;
...
-- SNAP: per-point nearest-edge snapping. LOCATIONS = ARRAY of [lon,lat] pairs,
-- RADIUS = search radius in meters. Returns the raw ORS VARIANT (:locations[]
-- entries are null or {location, name, snapped_distance}).
CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.CONTRACT.SNAP(METHOD VARCHAR, LOCATIONS ARRAY, RADIUS INT, REGION VARCHAR DEFAULT NULL, PROVIDER VARCHAR DEFAULT NULL)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$ ROUTING_PLATFORM.CONTRACT._DISPATCH_SNAP(METHOD, OBJECT_CONSTRUCT('locations', LOCATIONS, 'radius', RADIUS), REGION, PROVIDER) $$;

-- MATCH: map matching. FEATURES = a GeoJSON FeatureCollection VARIANT. Returns the
-- raw ORS response with :edge_ids per feature. For a road-following polyline use
-- OPENROUTESERVICE_APP.CORE.MATCH_PATH (chains /match + /export).
CREATE OR REPLACE FUNCTION ROUTING_PLATFORM.CONTRACT.MATCH(METHOD VARCHAR, FEATURES VARIANT, REGION VARCHAR DEFAULT NULL, PROVIDER VARCHAR DEFAULT NULL)
RETURNS VARIANT COMMENT='{"origin":"sf_sit-is-fleet","name":"oss-install-fleet-apps","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
AS $$ ROUTING_PLATFORM.CONTRACT._DISPATCH_MATCH(METHOD, OBJECT_CONSTRUCT('features', FEATURES), REGION, PROVIDER) $$;
