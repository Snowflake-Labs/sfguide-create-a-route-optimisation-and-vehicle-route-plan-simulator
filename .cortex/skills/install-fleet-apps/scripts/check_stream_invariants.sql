...
--
-- Run after any reseed. Non-zero rows in either result is a FAILURE, with ONE
-- documented exception - see KNOWN DIRTY below.
--
...
--
-- CONFIRMED after the generator fix, on a UsTexas dataset regenerated by the
-- FIXED generator (30 vehicles, 209 trips, 2026-09-01..08):
--   INVARIANT 1                    0 pairs   (was 42)
--   heartbeats inside a live trip  0         (was the artifact that split dwells)
--   INVARIANT 2                    0 rows in EVERY region
-- That is the empirical proof of the generator fix. It is also preset-agnostic:
-- the reservation, the UTC shift clock and the clamped end-of-day idle all sit in
-- the main vehicle-day path of studio/engine.ts, not behind a preset gate.
--
-- KNOWN DIRTY - READ BEFORE TREATING A FAILURE AS A REGRESSION.
-- UnitedStatesOfAmerica still returns 38 pairs / 20 vehicles (and 163 heartbeats
-- interleaved into live trips). That dataset PREDATES the generator fix and was
-- deliberately left un-regenerated: it holds only 21 site visits, so it is not
-- worth a full generation run. INVARIANT 1 therefore does NOT return zero rows
-- account-wide today, and the expected output is exactly one row, for that
-- region, with those numbers.
--
-- So the pass condition is: zero rows EXCEPT UnitedStatesOfAmerica at 38/20.
-- Any other region appearing, or those numbers changing, is a real regression.
-- Regenerating or dropping that dataset restores a clean zero-row signal, which
-- is strictly preferable - a check with a memorised exception is how a genuine
