...
if [ "${SKIP_ROUTING:-0}" != "1" ]; then
  ...
  if [ -f "$ROUTING_TOOLS_SQL" ]; then
    ...
    # Assert all 10 TOOL_* procs exist. Non-fatal by design (matches the
    ...
    TOOL_N=$(snow sql -c "$CONNECTION" --format=CSV -q \
      ...
      2>/dev/null | grep -Eo '^[0-9]+' | head -1 || echo 0)
    if [ "${TOOL_N:-0}" -lt 10 ]; then
      [ "$ROUTING_TOOLS_RC" -ne 0 ] && note "  WARN: ROUTING_TOOLS substrate reported errors; see /tmp/ifa_routing_tools.log"
      ROUTING_SUBSTRATE="DEGRADED: only ${TOOL_N:-0}/10 ROUTING_TOOLS.TOOL_* procs deployed - routing verbs will fail at agent runtime (see /tmp/ifa_routing_tools.log)"
      note "  WARN: $ROUTING_SUBSTRATE"
    else
      ROUTING_SUBSTRATE="OK (10/10 ROUTING_TOOLS.TOOL_* procs)"
