#!/usr/bin/env python3
"""
build_view_catalog.py - generate the SA deployment catalog SQL from the SA app views.

WHY THIS EXISTS
---------------
Every view carries a `useCase` block (Tenet 10, Channel D) and an
`agentKnowledge` block (Channel C). Until now those blocks were read ONLY by
...
so the in-app prefix and the agent-side catalog share ONE source of truth: the
view sources stay authoritative, and the table is a generated artifact.

TWO VIEW SOURCES
----------------
The SA app registers views two ways, and BOTH must reach the catalog:

  1. app-views.json                 - declarative area-views (19)
  2. packs/fleet/pack-views.json    - metadata for the code-registered Tier-3
                                      showcase views (5: the VRP simulator,
                                      emergency response, both backload views,
                                      and the ops console)

Reading only (1) was a real defect: the five showcase views - which are the whole
Optimization group - were absent from VIEW_CATALOG, so an agent asked "what can
you show me" outside the app could not name a single optimization demo. The pack
metadata was moved out of packs/fleet/index.ts into pack-views.json precisely so
this generator can read it without parsing TypeScript.

The starter pack (app/starter/app-views.json) is deliberately NOT read: it is a
separate deployment profile with its own agent spec, and listing its views here
would have the agent offer views this deployment does not have.
...
DEFAULT_PACK_VIEWS = (
    SKILL / "fleet_sa_app" / "ui" / "src" / "lib" / "packs" / "fleet" / "pack-views.json"
)
...
def load_views(views_path, pack_views_path):
    """Merge the declarative and code-registered view sources into one mapping.

    A view id present in both is a genuine authoring error (two components would
    claim the same nav entry), so fail rather than let one silently win.
    """
    views = json.loads(views_path.read_text())
    if pack_views_path and pack_views_path.exists():
        pack = json.loads(pack_views_path.read_text())
        clash = sorted(set(views) & set(pack))
        if clash:
            raise SystemExit(
                "ERROR: view id declared in BOTH app-views.json and pack-views.json: "
                + ", ".join(clash)
            )
        views.update(pack)
    return views
...
def rows_from(views_path, config_path, pack_views_path=None):
    views = load_views(views_path, pack_views_path)
    display = {}
...
def render(rows, views_path):
    ...
    out = [
        ...
        f"-- Source of truth: fleet_sa_app/app/{src} + "
        "fleet_sa_app/ui/src/lib/packs/fleet/pack-views.json",
        "-- (+ app-config.json display tokens).",
        "--",
        ...
        "-- does not exist. Regenerate after editing either view source:",
...
def main():
    ...
    rows = rows_from(
        views_path, pathlib.Path(args.config), pathlib.Path(args.pack_views)
    )
    ...
    if args.check:
        ...
        if out_path.read_text() != sql:
            print(
                f"FAIL: {out_path} is stale vs the view sources - "
