...
export default function DatasetsPanel() {
  ...
  const remove = async (id: string, region: string, vt: string) => {
    if (!confirm(`Permanently delete dataset for ${region} / ${vt}?\n\nThis purges all rows in DIM_POIS / DIM_FLEET / FACT_OFFERS / DIM_PARTNERS / FACT_PARTNER_HISTORY / FACT_TRIPS / FACT_VEHICLE_TELEMETRY where JOB_ID = ${id}.\n\nThis cannot be undone.`)) return;
    setBusy(id);
    try {
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.datasetId} style={{ background: r.isActive ? '#FAFCFE' : 'transparent' }}>
                  ...
                  <td style={{ ...cellStyle, textAlign: 'right' }}>{fmt(c.offers ?? c.deliveries)}</td>
