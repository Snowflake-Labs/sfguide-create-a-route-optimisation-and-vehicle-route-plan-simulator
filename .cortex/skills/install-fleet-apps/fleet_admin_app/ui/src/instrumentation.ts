...
export async function register(): Promise<void> {
  ...
  g.__fleetAdminBootInit = (async () => {
    // runSqlBatch throughout this file. Nothing here is interactive: it is
    // boot-time DDL convergence and a background reconciler, and no user is
    // waiting on any of it. Using runSql meant the container woke the INTERACTIVE
    // warehouse on every boot (thousands of CREATE OR REPLACE statements from
    // init.ts) and then again every 5 minutes, forever - measured 544 statements
    // / 24h, 272 wake-ups/day, ~19-20% duty cycle against a 60s AUTO_SUSPEND with
    // nobody using the app. Relocating it does not reduce that cost; it stops it
    // landing on the warehouse dashboards depend on.
    const { runSqlBatch } = await import('@/server/lib/sql');
    ...
    try {
      for (const db of ['FLEET_INTELLIGENCE', 'SYNTHETIC_DATASETS', 'OPENROUTESERVICE_APP', 'FLEET_APP', 'ROUTING_PLATFORM']) {
        try {
          await runSqlBatch(`ALTER DATABASE IF EXISTS ${db} SET DATA_RETENTION_TIME_IN_DAYS = 0`);
    ...
    try {
      await runSqlBatch(`ALTER COMPUTE POOL IF EXISTS OPENROUTESERVICE_APP_COMPUTE_POOL SET MIN_NODES = 1 MAX_NODES = 5`);
    } catch { /* pool may be mid-operation */ }
    try {
      await runSqlBatch(`ALTER SERVICE IF EXISTS OPENROUTESERVICE_APP.CORE.routing_gateway_service SET MIN_INSTANCES = 1 MAX_INSTANCES = 3`);
    ...
    try {
      ...
      await ensureBackloadAndAssetVelocityObjects(runSqlBatch);
    } catch (err) {
    ...
    try {
      ...
      await ensureObservabilityObjects(runSqlBatch);
    } catch (err) {
    ...
    try {
      ...
      const reconciled = await reconcileStaleJobs(runSqlBatch, null);
      if (reconciled > 0) {
        log('WARN', 'BootInit', `Reconciled ${reconciled} orphaned studio job(s)`);
        try {
          await runSqlBatch(`CALL OPENROUTESERVICE_APP.CORE.RECONCILE_AUTO_SUSPEND()`);
        } catch (reconcileErr) {
    ...
    setInterval(async () => {
      try {
        const { reconcileStaleJobs } = await import('@/server/studio/jobs');
        const n = await reconcileStaleJobs(runSqlBatch, 20);
        if (n > 0) {
