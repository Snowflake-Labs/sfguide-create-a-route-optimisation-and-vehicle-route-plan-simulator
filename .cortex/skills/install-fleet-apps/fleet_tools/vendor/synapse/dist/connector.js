...
export async function createConn(cfg) {
    const opts = {
        account: cfg.account,
 ...
 * the toml entry.
 */
export async function createConnFromCli(connectionName, overrides = {}) {
    ...
    const useStmts = [];
    if (overrides.role)
        useStmts.push(`USE ROLE ${overrides.role}`);
    if (overrides.warehouse)
        useStmts.push(`USE WAREHOUSE ${overrides.warehouse}`);
    if (overrides.database)
        useStmts.push(`USE DATABASE ${overrides.database}`);
    if (overrides.schema)
        useStmts.push(`USE SCHEMA ${overrides.schema}`);
...
function wrapSfConn(sfConn) {
    return {
        exec: async (sqlText, binds) => {
            const rows = await executeStatement(sfConn, sqlText, binds);
            return rows;
        },
        execRow: async (sqlText, binds) => {
            const rows = await executeStatement(sfConn, sqlText, binds);
            if (rows.length === 0)
                return null;
            return rows[0];
        },
        execScalar: async (sqlText, binds) => {
            const rows = await executeStatement(sfConn, sqlText, binds);
            if (rows.length === 0)
                return null;
            const first = rows[0];
...
async function connectAndWrap(opts, authenticator) {
