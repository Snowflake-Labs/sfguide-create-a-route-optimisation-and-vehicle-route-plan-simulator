...
export function parseTargetFlags(argv) {
    const out = {};
    for (let i = 0; i < argv.length; i++) {
        const a = argv[i];
        if (a === '--account' && i + 1 < argv.length) {
            out.account = argv[++i];
        }
        else if (a.startsWith('--account=')) {
            out.account = a.slice('--account='.length);
        }
        else if (a === '--install' && i + 1 < argv.length) {
            out.installPath = argv[++i];
        }
        else if (a.startsWith('--install=')) {
            out.installPath = a.slice('--install='.length);
        }
    }
    return out;
}
 ...
 */
export function resolveTargetDir(app, flags) {
