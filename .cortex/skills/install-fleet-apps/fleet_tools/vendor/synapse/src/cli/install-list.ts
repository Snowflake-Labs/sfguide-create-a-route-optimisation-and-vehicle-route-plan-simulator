 ...
 */
export async function runInstallList(app: ResolvedSynapseAppConfig, _argv: string[]): Promise<void> {
  // Walk up from app root to find apps/_installed.
  let workspaceRoot = app.appRoot;
  while (true) {
    if (fs.existsSync(path.join(workspaceRoot, 'apps', '_installed'))) break;
  ...
  const installs: Array<{ account: string; dir: string; cfg: ReturnType<typeof readInstallConfig> }> = [];
