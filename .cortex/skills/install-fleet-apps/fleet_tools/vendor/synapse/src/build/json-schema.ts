 ...
 */
export function toJsonSchema(schema: Schema<unknown>): Record<string, unknown> {
...
function baseSchema(schema: Schema<unknown>): Record<string, unknown> {
  ...
  switch (schema.kind) {
    ...
    case 'object': {
      ...
      for (const k of keys) properties[k] = toJsonSchema(shape[k]!);
