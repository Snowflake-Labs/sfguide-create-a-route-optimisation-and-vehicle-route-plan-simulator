...
function isProcDef(v: unknown): v is ProcDef<string, unknown, unknown> {
 ...
 */
export async function discoverProcs(procsDir: string): Promise<DiscoveredProcs> {
  if (!fs.existsSync(procsDir)) {
    throw new Error(`discoverProcs: no such directory ${procsDir}`);
  }
  const files = walkTs(procsDir).sort();
  const procs: Record<string, ProcDef<string, unknown, unknown>> = {};
  const paths: Record<string, string> = {};
  for (const file of files) {
    const mod = await import(url.pathToFileURL(file).href);
    for (const exported of Object.values(mod)) {
      if (!isProcDef(exported)) continue;
      const name = exported.name;
      if (procs[name]) {
        throw new Error(
          `discoverProcs: duplicate verb name "${name}" in ${file} ` +
          `and ${paths[name]}`,
        );
      }
      procs[name] = exported;
