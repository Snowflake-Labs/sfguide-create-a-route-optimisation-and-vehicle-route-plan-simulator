...
SELECT
    ...
    CEIL(
        ...
        END
        * CATCHMENT_POP.TOTAL_POP / 1000
        * f.UNITS_PER_1000
    ) AS FORECAST_UNITS,
    -- Actual varies: Mission = 85% fill (chronic under-supply), Market = 140% (over-ordering)
    CEIL(
        CASE
            WHEN p.PHARMACY_ID = 4 THEN 0.85   -- Mission: under-supplied
            WHEN p.PHARMACY_ID = 5 THEN 1.40   -- Market: over-stocked vs need
            WHEN p.PHARMACY_ID = 3 THEN 1.15   -- Clement: slightly over
            ELSE 1.05
        END
        * CEIL(
            CASE f.CONDITION
    ...
    CATCHMENT_POP.TOTAL_POP AS CATCHMENT_POPULATION,
    CASE f.CONDITION
...
JOIN FLEET_INTELLIGENCE.ROUTE_OPTIMIZATION.SF_HEALTH_DEMOGRAPHICS d
    ON d.NEIGHBORHOOD = (
        SELECT NEIGHBORHOOD FROM FLEET_INTELLIGENCE.ROUTE_OPTIMIZATION.SF_HEALTH_DEMOGRAPHICS
        ORDER BY SQRT(POW(d.LATITUDE - p.LATITUDE, 2) + POW(d.LONGITUDE - p.LONGITUDE, 2))
        LIMIT 1
    )
JOIN (
    SELECT
...
) CATCHMENT_POP ON CATCHMENT_POP.PHARMACY_ID = p.PHARMACY_ID;
...
SELECT 'SF_INVENTORY'             AS tbl, COUNT(*) AS rows FROM FLEET_INTELLIGENCE.ROUTE_OPTIMIZATION.SF_INVENTORY
UNION ALL
