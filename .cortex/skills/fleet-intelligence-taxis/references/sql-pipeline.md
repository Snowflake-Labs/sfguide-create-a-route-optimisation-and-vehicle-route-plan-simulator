...
These views read from `SYNTHETIC_DATASETS.UNIFIED`, filtered by the CONFIG table (vehicle type + region). Create these views so Data Studio-generated data is accessible alongside the skill's own pipeline.
...
USING (SELECT 'car' AS VEHICLE_TYPE, 'SanFrancisco' AS REGION) src
ON TRUE
