...
> **_GEO Alternative:** You can combine steps 7a + 7b into a single step using `DIRECTIONS_GEO`:
> ```sql
...
These views allow the skill to read from the unified Data Studio tables (`SYNTHETIC_DATASETS.UNIFIED`), filtered to car/taxi data only. Create these views so Data Studio–generated data is accessible alongside the skill's own pipeline.
...
SELECT
    ...
    DESTINATION_LAT,
    DESTINATION_LON,
    ROUTE_GEOG AS GEOMETRY,
    DISTANCE_KM,
    DURATION_MINUTES,
    TRIP_START AS PICKUP_TIME,
    TRIP_END AS DROPOFF_TIME,
    STATUS,
    ORS_PROFILE,
    REGION
