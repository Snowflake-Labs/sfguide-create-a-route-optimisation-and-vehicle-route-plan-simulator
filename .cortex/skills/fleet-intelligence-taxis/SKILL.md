...
> 2. **Always use fully qualified object names** (`OPENROUTESERVICE_SETUP.FLEET_INTELLIGENCE_TAXIS.<object>`). Session context from `USE` statements does not persist across calls.
...
Create `OPENROUTESERVICE_SETUP` database, `ROUTING_ANALYTICS` warehouse, `FLEET_INTELLIGENCE_TAXIS` schema, and `STREAMLIT_STAGE` stage.
