...
# Get summary stats
try:
    trips_count = session.table('FLEET_INTELLIGENCE.FLEET_INTELLIGENCE_TAXIS.TRIPS_ASSIGNED_TO_DRIVERS').count()
    drivers_count = session.table('FLEET_INTELLIGENCE.FLEET_INTELLIGENCE_TAXIS.TRIPS_ASSIGNED_TO_DRIVERS').select('DRIVER_ID').distinct().count()
    locations_count = session.table('FLEET_INTELLIGENCE.FLEET_INTELLIGENCE_TAXIS.TAXI_LOCATIONS').count()
    
    # Get total distance
    total_distance = session.table('FLEET_INTELLIGENCE.FLEET_INTELLIGENCE_TAXIS.TRIP_SUMMARY').agg(
