...
> - R2.1: DONE — scope-arg data contract. `fleet_sa_app/app/scoped_contract.sql` + mirrored into control-app `init.ts` boot path (new-deployment-first). 4 fns live: `F_{FACT_TRIPS,FACT_VEHICLE_TELEMETRY,DIM_POIS,DIM_FLEET}_SCOPED(region,dataset_id)`. VERIFIED multi-tenant: dataset b64491a0->394 trips vs 7d361705->0 (isolation); NULL-fallback==global V_*_CURRENT==394 (back-compat); fn==raw base filter (correctness). Server tsc green. Remaining tables (trip_schedule, freight_offers, partners, partner_history, v_places, offer_routes) = same pattern, follow-on.
> - R2.2: PENDING (needs deploy-verify, batch with R1.2) — consumer wiring: add `dataset_id` to contextBar, thread `region`+`dataset_id` as `:params` via use-view-data, repoint dashboards to `F_*_SCOPED`, and make consumer "switch" STOP writing CONFIG (read-only per-session). Verify with two concurrent browser sessions on different scopes.
