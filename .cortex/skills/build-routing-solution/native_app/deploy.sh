...
snow sql -c "$CONN" -q "GRANT INSERT ON ALL TABLES IN DATABASE FLEET_INTELLIGENCE TO APPLICATION OPENROUTESERVICE_NATIVE_APP;"
snow sql -c "$CONN" -q "GRANT UPDATE ON ALL TABLES IN DATABASE FLEET_INTELLIGENCE TO APPLICATION OPENROUTESERVICE_NATIVE_APP;"
snow sql -c "$CONN" -q "GRANT DELETE ON ALL TABLES IN DATABASE FLEET_INTELLIGENCE TO APPLICATION OPENROUTESERVICE_NATIVE_APP;"

echo "==> Ensuring JOB_ID column on UNIFIED tables..."
for TBL in FACT_VEHICLE_TELEMETRY FACT_TRIPS DIM_FLEET DIM_POIS DIM_TRIP_SCHEDULE; do
  snow sql -c "$CONN" -q "ALTER TABLE SYNTHETIC_DATASETS.UNIFIED.${TBL} ADD COLUMN IF NOT EXISTS JOB_ID VARCHAR;" 2>/dev/null || true
done
