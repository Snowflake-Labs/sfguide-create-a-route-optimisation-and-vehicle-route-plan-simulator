...
RETURNS STRING
LANGUAGE SQL
COMMENT = '{"origin":"sf_sit-is-fleet","name":"build-routing-solution","version":"2.0","attributes":{"component":"core"}}'
AS
$$
DECLARE
  fn_name VARCHAR;
  ...
  c CURSOR FOR
    ...
    WHERE FUNCTION_SCHEMA = 'CORE'
      AND (
        ...
        OR FUNCTION_NAME LIKE '%\_GEO'
      );
BEGIN
  FOR fn IN c DO
    LET clean_sig VARCHAR := REGEXP_REPLACE(fn.ARGUMENT_SIGNATURE, '[A-Z_]+ (VARCHAR|VARIANT|ARRAY|FLOAT|NUMBER|INT|TEXT|BOOLEAN|GEOGRAPHY)', '\\1');
    drop_sql := 'DROP FUNCTION IF EXISTS core.' || fn.FUNCTION_NAME || clean_sig;
    EXECUTE IMMEDIATE drop_sql;
  END FOR;
  BEGIN DROP FUNCTION IF EXISTS core.DIRECTIONS(VARCHAR, ARRAY, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.DIRECTIONS(VARCHAR, VARIANT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.ISOCHRONES(TEXT, FLOAT, FLOAT, INT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.OPTIMIZATION(ARRAY, ARRAY, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.OPTIMIZATION(VARIANT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.ORS_STATUS(); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.MATRIX(VARCHAR, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.MATRIX(VARCHAR, VARIANT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.MATRIX_TABULAR(VARCHAR, ARRAY, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.DIRECTIONS(VARCHAR, VARCHAR, ARRAY, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.DIRECTIONS(VARCHAR, VARCHAR, VARIANT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.ISOCHRONES(VARCHAR, TEXT, FLOAT, FLOAT, INT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.MATRIX(VARCHAR, VARCHAR, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.MATRIX(VARCHAR, VARCHAR, VARIANT); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.MATRIX_TABULAR(VARCHAR, VARCHAR, ARRAY, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  BEGIN DROP FUNCTION IF EXISTS core.OPTIMIZATION(VARCHAR, ARRAY, ARRAY, ARRAY); EXCEPTION WHEN OTHER THEN NULL; END;
  RETURN 'Legacy functions cleaned up (v2.0 consolidation)';
EXCEPTION
