...
export default function CatchmentPanel() {
  ...
  useEffect(() => {
    setLoading(true);
    sfQuery(`SELECT RESTAURANT_ID, RESTAURANT_NAME, ST_X(LOCATION) AS LNG, ST_Y(LOCATION) AS LAT, TOTAL_ORDERS, AVG_DELIVERY_TIME_MIN FROM RESTAURANTS ORDER BY TOTAL_ORDERS DESC LIMIT 100`)
      .then(setRestaurants)
