...
export default function Intro() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res1 = await fetch('/api/query', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sql: `SELECT VALUE::STRING AS H3_INDEX FROM TABLE(FLATTEN(H3_COVERAGE_STRINGS(TO_GEOGRAPHY('POLYGON((-122.520 37.700, -122.350 37.700, -122.350 37.820, -122.520 37.820, -122.520 37.700))'), 8)))`, database: INTRO_DB, schema: INTRO_SCHEMA }) });
      ...
      setHexLoading(false);
      try {
        const res2 = await fetch('/api/query', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sql: `SELECT TRIP_ID, O_LNG, O_LAT, D_LNG, D_LAT, ROUND(DISTANCE_M, 0) AS DISTANCE_M, ROUND(DURATION_S, 0) AS DURATION_S, ROUTE_GEOJSON::STRING AS ROUTE_GEOJSON FROM INTRO_TRIPS ORDER BY TRIP_ID`, database: INTRO_DB, schema: INTRO_SCHEMA }) });
  ...
  const tripLayer = useMemo(() => {
    ...
    return new PathLayer({
      ...
      getColor: (d: any) => {
        ...
        const idx = Math.min(Math.floor(t * TRIP_COLORS.length), TRIP_COLORS.length - 1);
        return [...TRIP_COLORS[idx], 180] as [number, number, number, number];
      },
      getWidth: 2,
      widthMinPixels: 1,
      capRounded: true,
  ...
  const gpsPingsLayer = useMemo(() => {
    ...
    return new ScatterplotLayer({
      ...
      getFillColor: (d: any) => {
        ...
        const idx = Math.min(Math.floor(norm * TRIP_COLORS.length), TRIP_COLORS.length - 1);
        return [...TRIP_COLORS[idx], 200] as [number, number, number, number];
      },
      getRadius: 8,
      radiusMinPixels: 1.5,
      radiusMaxPixels: 6,
  ...
  const layers = useMemo(() => [basemap, hexLayer, gpsPingsLayer, tripLayer, originLayer, destLayer].filter(Boolean), [basemap, hexLayer, gpsPingsLayer, tripLayer, originLayer, destLayer]);
