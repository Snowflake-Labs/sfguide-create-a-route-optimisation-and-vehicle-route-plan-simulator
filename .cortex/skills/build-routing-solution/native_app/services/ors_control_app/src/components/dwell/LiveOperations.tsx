...
export default function LiveOperations() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [v, d] = await Promise.all([
      sfQuery(`SELECT VEHICLE_ID AS DRIVER_ID, STATUS AS CURRENT_STATE, LONGITUDE AS LNG, LATITUDE AS LAT, TS AS LAST_UPDATE, SPEED_KMH AS CURRENT_SPEED_KMH FROM DT_STATE_CHANGES WHERE IS_STATE_CHANGE = TRUE QUALIFY ROW_NUMBER() OVER (PARTITION BY VEHICLE_ID ORDER BY TS DESC) = 1 LIMIT 500`),
