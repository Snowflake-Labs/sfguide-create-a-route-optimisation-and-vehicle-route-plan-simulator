        ...
        <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {trips.map((t: any) => (
              <tr
                key={t.TRIP_ID}
                onClick={() => loadTrip(t.DRIVER_ID)}
                style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer', background: selectedTrip === t.DRIVER_ID ? 'rgba(41,181,232,0.1)' : 'transparent' }}
              >
                <td style={{ padding: '6px 8px', fontFamily: 'monospace', fontSize: 11 }}>{String(t.TRIP_ID).slice(-12)}</td>
                <td style={{ padding: '6px 8px' }}>{t.DRIVER_ID}</td>
                <td style={{ padding: '6px 8px' }}>{t.DWELL_COUNT}</td>
                <td style={{ padding: '6px 8px' }}>{t.TOTAL_DWELL_MIN}</td>
              </tr>
            ))}
          </tbody>
        </table>
