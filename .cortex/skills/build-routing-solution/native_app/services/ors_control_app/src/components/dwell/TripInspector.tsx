...
export default function TripInspector() {
  ...
  useEffect(() => {
    ...
    setError(null);
    sfQuery(
      `SELECT TRIP_ID, TRUCK_ID AS DRIVER_ID, MIN(SESSION_START) AS START_TIME, MAX(SESSION_END) AS END_TIME, COUNT(*) AS DWELL_COUNT, ROUND(SUM(DWELL_MINUTES), 1) AS TOTAL_DWELL_MIN FROM DT_DWELL_ENRICHED WHERE STATUS LIKE 'DWELL%' GROUP BY TRIP_ID, TRUCK_ID ORDER BY START_TIME DESC LIMIT 50`
    ).then(rows => {
      setTrips(rows);
