...
export default function FleetOverview() {
  const { data: kpiRows, loading: kpiLoading } = useSfQuery(
    `SELECT COUNT(DISTINCT DRIVER_ID) AS DRIVERS, COUNT(DISTINCT TRIP_ID) AS TRIPS, ROUND(AVG(ROUTE_DISTANCE_METERS / 1000), 1) AS AVG_DISTANCE_KM, ROUND(AVG(ROUTE_DURATION_SECS / 60), 1) AS AVG_DURATION_MIN FROM TRIP_SUMMARY`,
    FT_DB, FT_SCHEMA,
  );
  const { data: trips, loading: tripsLoading } = useSfQuery(
    `SELECT TRIP_ID, DRIVER_ID, ST_X(ORIGIN) AS P_LNG, ST_Y(ORIGIN) AS P_LAT, ST_X(DESTINATION) AS D_LNG, ST_Y(DESTINATION) AS D_LAT, ROUND(ROUTE_DISTANCE_METERS / 1000, 2) AS TRIP_DISTANCE_KM, ROUND(ROUTE_DURATION_SECS / 60, 1) AS TRIP_DURATION_MIN FROM TRIP_SUMMARY ORDER BY TRIP_START_TIME DESC LIMIT 200`,
    FT_DB, FT_SCHEMA,
  );
  const { data: hourly } = useSfQuery(
    `SELECT HOUR(TRIP_START_TIME) AS HOUR, COUNT(*) AS TRIPS FROM TRIP_SUMMARY GROUP BY 1 ORDER BY 1`,
    FT_DB, FT_SCHEMA,
  );
  ...
  return (
    <div className="page-dashboard">
      ...
      <div style={{ height: 500, borderRadius: 8, border: '1px solid var(--border)', overflow: 'hidden', position: 'relative', background: '#e8e8e8', marginBottom: 20 }}>
