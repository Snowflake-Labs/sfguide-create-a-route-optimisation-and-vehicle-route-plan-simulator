...
export default function FDMatrixBuilder() {
  ...
  const { data: registry, loading: regLoading } = useSfQuery(
    `SELECT REGION_NAME, H3_RESOLUTION, TOTAL_HEXES, STATUS, CREATED_AT FROM MATRIX_REGISTRY ORDER BY CREATED_AT DESC LIMIT 20`,
    FD_DB, FD_SCHEMA,
  );
  const { data: hexData, loading: hexLoading } = useSfQuery(
    `SELECT H3_INDEX, TRAVEL_TIME_MIN FROM TRAVEL_TIME_MATRIX LIMIT 5000`,
    FD_DB, FD_SCHEMA,
  );

  const loading = regLoading || hexLoading;
  ...
  return (
    <div className="page-full">
      <div className="page-sidebar-panel">
        ...
        <p>Travel time matrix registry and visualization</p>
