...
export default function HeatMap() {
  ...
  const getTooltip = useCallback(({ object }: any) => {
    ...
    return {
      html: `<b>${object.H3_INDEX}</b><br/>Trips: ${object.TRIP_COUNT}<br/>Avg Speed: ${object.AVG_SPEED} km/h`,
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ fontSize: 20, margin: 0 }}>Heat Map</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: 13, margin: 0 }}>{loading ? 'Loading...' : `${hexData.length} hexagons · ${totalTrips.toLocaleString()} trips`}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <div style={{ minWidth: 180 }}>
          <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Hour: {hour < 0 ? 'All' : `${hour}:00`}</label>
          <input type="range" min={-1} max={23} value={hour} onChange={e => setHour(Number(e.target.value))} style={{ width: '100%' }} />
        </div>
        <div style={{ minWidth: 180 }}>
          <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>H3 Resolution: {h3Res}</label>
