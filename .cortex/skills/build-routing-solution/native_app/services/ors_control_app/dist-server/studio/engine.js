...
function mapCategoryToType(category, mode) {
    if (mode === 'food_delivery') {
        if (['restaurant', 'fast_food', 'cafe', 'bakery', 'pizza', 'sushi'].includes(category))
            return 'RESTAURANT';
        return 'ADDRESS';
    }
    if (mode === 'trucking') {
        if (['warehouse', 'industrial', 'logistics'].includes(category))
            return 'WAREHOUSE';
        if (['fuel', 'rest_area', 'parking'].includes(category))
            return 'REST_STOP';
        return 'DESTINATION';
    }
    return 'LOCATION';
}
function generateSyntheticPOIs(config) {
