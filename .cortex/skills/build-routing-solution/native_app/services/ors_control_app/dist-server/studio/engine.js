...
export async function loadPOIs(config, snowSql) {
    ...
    try {
        ...
        throw new Error(`No POIs found in Overture Maps for region bbox ` +
            `[${bbox.min_lat},${bbox.min_lng} to ${bbox.max_lat},${bbox.max_lng}] ` +
            `with categories [${cats.join(', ')}]. Expand the bbox or change categories.`);
    }
    catch (e) {
        if (e.message?.startsWith('No POIs found'))
            throw e;
        log('ERROR', 'Studio', `Overture Maps query failed`, {
            detail: { error: e.message?.slice(0, 200), bbox, categories: cats },
        });
        throw new Error(`Cannot load POIs: Overture Maps is not accessible. ` +
            `Ensure the OVERTURE_MAPS__PLACES share is mounted. Error: ${e.message?.slice(0, 200)}`);
    }
}
function mapCategoryToType(category, mode) {
    if (mode === 'food_delivery') {
        if (['restaurant', 'fast_food_restaurant', 'cafe', 'bakery', 'pizzaria', 'casual_eatery', 'coffee_shop', 'sandwich_shop', 'chicken_restaurant'].includes(category))
            return 'RESTAURANT';
        return 'ADDRESS';
    }
    if (mode === 'trucking') {
        if (['warehouse', 'storage_facility', 'b2b_transportation_and_storage_service', 'industrial_facility_or_service'].includes(category))
            return 'WAREHOUSE';
        if (['gas_station', 'parking', 'transportation_location', 'ground_transport_facility_or_service'].includes(category))
            return 'REST_STOP';
        return 'DESTINATION';
    }
    return 'LOCATION';
}
export function buildFleet(config, pois, rng) {
    const fleet = [];
    const { num_vehicles } = config.fleet;
    const profiles = Object.entries(config.driver_profiles);
    const homePois = pois.filter(p => config.mode === 'food_delivery' ? p.location_type === 'RESTAURANT' :
...
function pickDestination(origin, pois, config, rng) {
    ...
    if (r < short_pct)
        maxKm = short_max_km;
    else if (r < short_pct + medium_pct)
        maxKm = medium_max_km;
    else
        maxKm = 999;
...
export async function* generateTelemetry(config, snowSql, onProgress, abortSignal) {
    ...
    for (const m of fleet)
        profileBreakdown[m.profile_type] = (profileBreakdown[m.profile_type] || 0) + 1;
        ...
        for (const member of fleet) {
            ...
            for (let t = 0; t < numTrips; t++) {
                ...
                if (config.breaks && lifecycle.minSinceBreak >= config.breaks.driving_hours_between_breaks * 60) {
                    const breakDwell = { median_min: config.breaks.mandatory_break_duration_min, sigma: 0.2, max_min: config.breaks.mandatory_break_duration_min * 1.5 };
