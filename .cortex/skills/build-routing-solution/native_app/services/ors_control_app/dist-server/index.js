...
app.get('/api/regions/catalog', async (req, res) => {
    try {
        ...
        let where = 'WHERE 1=1';
        if (search)
            where += ` AND LOWER(REGION_NAME) LIKE '%${escapeString(search.toLowerCase())}%'`;
        if (source)
            where += ` AND SOURCE = '${escapeString(source)}'`;
        if (level)
            where += ` AND LEVEL = '${escapeString(level)}'`;
...
app.get('/api/regions/:region/build-progress', async (req, res) => {
    try {
        ...
        const rows = await runSql(`SELECT SYSTEM$GET_SERVICE_LOGS('${svcName}', 0, 'ors', 500) AS LOGS`);
        ...
        const parseNum = (s) => parseInt(s.replace(/\s/g, ''), 10);
