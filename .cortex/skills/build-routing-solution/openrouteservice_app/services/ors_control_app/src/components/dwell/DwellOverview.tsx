...
export default function DwellOverview() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      sfQuery(`SELECT COUNT(DISTINCT SESSION_ID) AS TOTAL_TRIPS, ROUND(AVG(DWELL_MINUTES),1) AS AVG_DWELL, ROUND(SUM(CASE WHEN DWELL_MINUTES <= 30 THEN 1 ELSE 0 END)*100.0/NULLIF(COUNT(*),0),1) AS SLA_PCT, COUNT(DISTINCT VEHICLE_ID) AS ACTIVE_DRIVERS FROM DT_DWELL_ENRICHED`),
