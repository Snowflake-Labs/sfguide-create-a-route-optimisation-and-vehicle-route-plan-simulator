...
export default function CourierHeatmap() {
  ...
  const hexLayer = useMemo(() => {
    ...
    return new H3HexagonLayer({
      id: 'courier-heatmap',
      data: validData,
      pickable: true, filled: true, extruded: true, elevationScale: 50,
  ...
  const getTooltip = useCallback(({ object }: any) => {
    ...
    return { html: `<b>${object.H3_INDEX}</b><br/>Pings: ${object.PING_COUNT}<br/>Avg Speed: ${fmtDec(object.AVG_SPEED)} km/h`, style: { backgroundColor: '#14141f', color: '#e8e8f0', padding: '8px', borderRadius: '4px', fontSize: '12px' } };
  }, []);

  return (
    <div className="panel">
      <h2 style={{ fontSize: 20, marginBottom: 4 }}>Courier Heatmap</h2>
      <p className="subtitle">{loading ? 'Loading...' : `${hexData.length} hexagons · ${totalPings.toLocaleString()} pings`}</p>
      <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        <div className="form-group" style={{ minWidth: 120 }}>
          <label>Metric</label>
          <select className="form-select" value={metric} onChange={e => setMetric(e.target.value as any)}>
            <option value="PING_COUNT">Ping Count</option>
