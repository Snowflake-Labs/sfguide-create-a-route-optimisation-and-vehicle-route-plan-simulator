...
export default function TripInspector() {
  ...
  const loadTrip = useCallback(async (tripId: string) => {
    ...
    const [points, dwells] = await Promise.all([
      sfQuery(`SELECT ST_X(POINT_GEOM) AS LNG, ST_Y(POINT_GEOM) AS LAT, TS AS EVENT_TIMESTAMP, SPEED_KMH FROM DT_STATE_CHANGES WHERE TRIP_ID = '${tripId}' ORDER BY TS`),
      sfQuery(`SELECT ST_X(AVG_POINT) AS LNG, ST_Y(AVG_POINT) AS LAT, LOCATION_NAME AS FACILITY_NAME, ROUND(DWELL_MINUTES, 1) AS DWELL_MIN, CASE WHEN DWELL_MINUTES > 30 THEN 'BREACH' ELSE 'OK' END AS SLA_STATUS FROM DT_DWELL_ENRICHED WHERE TRIP_ID = '${tripId}'`),
    ]);
