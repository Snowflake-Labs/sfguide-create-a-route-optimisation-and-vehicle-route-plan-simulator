...
export function createRegionsCatalogRouter(): Router {
  ...
  router.get('/api/regions/catalog/:catalogId/boundary', async (req, res) => {
    try {
      const catalogId = escapeString(String(req.params.catalogId || '').trim());
      if (!catalogId) {
        res.status(400).json({ boundaryGeoJson: null, error: 'catalogId required' });
        return;
      }
      ...
      const row = rows?.[0];
      res.json({ boundaryGeoJson: row?.BOUNDARY_GEOJSON ?? null });
    } catch (err: any) {
      res.status(500).json({ boundaryGeoJson: null, error: err.message });
    }
  });
