...
BEGIN
    ...
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT('batch_id',BATCH_ID,'product_id',PRODUCT_ID,'status',STATUS,'batch_value_usd',BATCH_VALUE_USD,'deviation_count',DEVIATION_COUNT,'deviation_severity',DEVIATION_SEVERITY,'qc_result',QC_RESULT,'start_date',START_DATE::VARCHAR,'target_completion',TARGET_COMPLETION::VARCHAR)) INTO v_batches FROM FLEET_INTELLIGENCE.PHARMA_SUPPLY_CHAIN.PRODUCTION_BATCHES WHERE PLANT_ID=:v_plant_id;
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT('material',MATERIAL_NAME,'type',MATERIAL_TYPE,'quantity_kg',QUANTITY_KG,'days_coverage',DAYS_OF_COVERAGE,'stock_status',STOCK_STATUS,'temp_excursion',TEMP_EXCURSION_FLAG,'storage',STORAGE_CONDITION)) INTO v_inventory FROM FLEET_INTELLIGENCE.PHARMA_SUPPLY_CHAIN.MATERIAL_INVENTORY WHERE PLANT_ID=:v_plant_id;
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT('shipment_id',SHIPMENT_ID,'supplier_id',SUPPLIER_ID,'material_type',MATERIAL_TYPE,'value_usd',VALUE_USD,'status',STATUS,'delay_days',DELAY_DAYS,'temp_excursion',TEMPERATURE_EXCURSION,'customs_hold',CUSTOMS_HOLD,'carrier',CARRIER,'expected',EXPECTED_ARRIVAL::VARCHAR)) INTO v_shipments FROM FLEET_INTELLIGENCE.PHARMA_SUPPLY_CHAIN.SHIPMENTS WHERE PLANT_ID=:v_plant_id;
