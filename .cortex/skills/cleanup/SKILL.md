...
Set the session query tag, then run the discovery queries from [`references/discovery-queries.sql`](references/discovery-queries.sql). This searches across all accessible databases for objects tagged with the tracking origin.

The discovery queries cover: tables, views, procedures/functions, stages, warehouses, schemas, databases, and Streamlit apps. Each query filters by `COMMENT LIKE '%sf_sit-is-fleet%'`.
