...
try:
    city_orders = session.table('FLEET_INTELLIGENCE.FLEET_INTELLIGENCE_FOOD_DELIVERY.ORDERS_ASSIGNED_TO_COURIERS').filter(F.col('CITY') == selected_city)
    city_summary = session.table('FLEET_INTELLIGENCE.FLEET_INTELLIGENCE_FOOD_DELIVERY.DELIVERY_SUMMARY').filter(F.col('CITY') == selected_city)
