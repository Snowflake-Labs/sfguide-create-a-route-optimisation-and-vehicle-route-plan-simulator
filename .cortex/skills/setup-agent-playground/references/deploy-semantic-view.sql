...
CREATE OR REPLACE SEMANTIC VIEW FLEET_INTELLIGENCE.ROUTING_AGENT.FLEET_ANALYTICS_VIEW
  ...
  FACTS (
    ...
    trips.duration_minutes AS DURATION_MINUTES COMMENT = 'Trip duration in minutes',
    trips.detour_distance_km AS DETOUR_DISTANCE_KM COMMENT = 'Extra distance from route deviation in km',
    telemetry.speed_kmh AS SPEED_KMH COMMENT = 'Vehicle speed km/h',
    telemetry.battery_pct AS BATTERY_PCT COMMENT = 'Battery charge percentage 0-100',
    telemetry.odometer_km AS ODOMETER_KM COMMENT = 'Cumulative distance driven km'
  )
  DIMENSIONS (
    ...
    trips.status AS trips.STATUS WITH SYNONYMS = ('trip status', 'delivery status') COMMENT = 'Trip status: COMPLETED',
    ...
    fleet.vehicle_id AS fleet.VEHICLE_ID WITH SYNONYMS = ('driver', 'courier', 'vehicle') COMMENT = 'Vehicle/driver identifier',
    fleet.driver_profile AS fleet.DRIVER_PROFILE WITH SYNONYMS = ('driver type', 'behaviour') COMMENT = 'Driver profile type',
    pois.name AS pois.NAME WITH SYNONYMS = ('restaurant', 'place', 'POI', 'location') COMMENT = 'Point of interest name',
    pois.category AS pois.CATEGORY WITH SYNONYMS = ('POI type', 'business type') COMMENT = 'POI category',
    pois.location_type AS pois.LOCATION_TYPE WITH SYNONYMS = ('POI classification') COMMENT = 'Location classification',
    telemetry.status AS telemetry.STATUS WITH SYNONYMS = ('state', 'activity', 'what is vehicle doing') COMMENT = 'MOVING, DWELL_ORIGIN, DWELL_DESTINATION, DWELL_RECHARGE, IDLE',
    ...
    telemetry.is_detour AS telemetry.IS_DETOUR WITH SYNONYMS = ('off route point') COMMENT = 'Whether GPS point was off planned route',
  ...
  METRICS (
    ...
    trips.avg_duration_min AS AVG(trips.duration_minutes) WITH SYNONYMS = ('average duration', 'delivery time') COMMENT = 'Average trip duration minutes',
    ...
    telemetry.avg_speed AS AVG(telemetry.speed_kmh) WITH SYNONYMS = ('average speed', 'mean speed') COMMENT = 'Average vehicle speed km/h',
    ...
    telemetry.avg_battery AS AVG(telemetry.battery_pct) WITH SYNONYMS = ('average battery', 'mean charge') COMMENT = 'Average battery percentage'
  )
  ...
  AI_SQL_GENERATION 'Round numeric results to 1 decimal. Use TRIM(pois.NAME, chr(34)) for POI names. For percentages multiply by 100 and add %. Group by VEHICLE_ID for vehicle comparison.'
  AI_VERIFIED_QUERIES (
    trips_by_day AS (
